...
class BertSST2Model(nn.Module):
    ...
    def __init__(self, class_size, pretrained_name='bert-base-chinese'):
        ...
        self.bert = BertModel.from_pretrained(pretrained_name,
                                              return_dict=True)
        # 通过三个线性层将[CLS]标签对应的维度：768->class_size
    ...
    def forward(self, inputs):
        ...
        categories_numberic = self.classifier(self.relu(output.pooler_output))
        return categories_numberic

def save_pretrained(model , path):
    # 保存模型，先利用os模块创建文件夹，后利用torch.save()写入模型文件
    os.makedirs(path , exist_ok=True)
    torch.save(model , os.path.join(path , 'BertSST2.pth')) #这个保存着太大了

def load_sentence(data_path, train_ratio=0.8):
    all_data = []
...
# 获取训练、测试数据
train_data, test_data = load_sentence(
...
# 创建模型 BertSST2Model
model = BertSST2Model(len(categories) , pretrain_model_name)
model.to(device)
# 加载预训练模型对应的tokenizer
...
def train_and_value(discribe):
    for epoch in range(1, num_epoch + 1):
        ...
        for batch in tqdm(test_dataloader, desc=f"Testing"):
            ...
            with torch.no_grad():
                ...
                """
                    ...
                    这里用argmax选算出来的2元组中较大的那个，作为最后的预测结果
                """
        ...
        #     save_pretrained(model, checkpoints_dirname + f'/{discribe}-checkpoints-{epoch}/{num_epoch}')
...
for name , child in model.named_children():
    if name in ['linear_1' , 'linear_2' , 'classifier']:
        # 将全连接层设为训练模式
        child.train()
    else:
        child.train()

train_and_value("bertLocked")
